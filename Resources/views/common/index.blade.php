                        ...
                        <tbody>
                            ...
                            @foreach($items as $item)
                                <tr align="center" class="odd">
                                    ...
                                    <td nowrap="nowrap">
                                        <a class="btn btn-icon-only blue" href="{{ route($module.'.edit', $item->id) }}" title="<?=_('Редактировать')?>"><i class="fa fa-pencil"></i></a>
                                        <form id="{{ $module.$item->id }}" action="{{ route($module.'.destroy', $item->id) }}" method="POST" style="display: none;">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                        </form>
                                        <a href="javascript:del('{{ $module.$item->id }}')" title="<?=_('Удалить')?>" class="btn btn-icon-only red"><i class="fa fa-trash-o"></i></a>
                                    </td>
                                </tr>
                            @endforeach
